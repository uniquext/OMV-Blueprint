...
# ==============================================================================
# 当前脚本作用：将本地照片目录扁平化后，通过 immich-cli Docker 镜像上传至 Immich 服务器。
# 核心逻辑：检查环境 -> 目录扁平化 -> 模拟运行 -> 用户确认 -> 正式上传。
# 上传方式：以源目录名作为相册名，通过 Docker 容器挂载只读目录执行 immich-cli upload。
#
# 用法：bash immich_ingest.sh [-d 源目录] [-s IP:端口] [-k API_KEY]
# 参数：
#   -d [源目录]        待上传的本地照片目录路径
#   -s [IP:端口]       Immich 服务器地址（例：192.168.1.100:2283）
#   -k [API_KEY]       Immich API 密钥
# 示例：
#   bash immich_ingest.sh -d /mnt/Cache/澄宝宝 -s 192.168.1.100:2283 -k your_api_key
#   bash immich_ingest.sh -d /mnt/Cache/澄宝宝
# ==============================================================================
...
while getopts "d:s:k:" opt; do
    case $opt in
        ...
        *)
            echo "❌ 未知参数: -$OPTARG"
            echo "用法: bash immich_ingest.sh [-d 源目录] [-s IP:端口] [-k API_KEY]"
            exit 1
            ;;
    esac
done
...
# --- 步骤 3：确认正式提交 ---
read -p "⚠️  模拟运行结束。是否开始正式入库? (y/n): " confirm
