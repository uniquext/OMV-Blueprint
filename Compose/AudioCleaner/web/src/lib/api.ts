...
export const api = {
  ...
  historyPage: async (page?: PageRequest) =>
	...
	recovery: () => request<RecoveryItem[]>('/api/recovery'),
	failures: () => request<FailureIssue[]>('/api/failures'),
	retainRecovery: (id: number) => request<OperationStatusResponse>(`/api/recovery/${id}/retain`, post()),
	restoreRecovery: (id: number) => request<RestoreResult>(`/api/recovery/${id}/restore`, post()),
	deleteRecoveryBackup: (id: number) => request<OperationStatusResponse>(`/api/recovery/${id}/backup`, { method: 'DELETE' }),
	retryHistoryContext: (id: number) => request<Record<string, unknown>>(`/api/history/${id}/retry-context`),
	retryFileContext: (id: number) => request<RetryContext>(`/api/files/${id}/retry-context`),
	retryRecoveryContext: (id: number) => request<RetryContext>(`/api/recovery/${id}/retry-context`),
	retryRecovery: (id: number) => request<OperationStatusResponse>(`/api/recovery/${id}/retry`, post()),
}
