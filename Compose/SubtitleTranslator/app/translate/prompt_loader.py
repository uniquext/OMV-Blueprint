...
MT_SYSTEM_PROMPT_PATH = "/app/prompts/system_prompt_mt.txt"
...
def load_mt_system_prompt() -> str:
    if not os.path.isfile(MT_SYSTEM_PROMPT_PATH):
        raise FileNotFoundError(f"MT system prompt file not found: {MT_SYSTEM_PROMPT_PATH}")
    with open(MT_SYSTEM_PROMPT_PATH, "r", encoding="utf-8") as f:
        return f.read()
