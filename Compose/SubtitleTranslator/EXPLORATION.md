...
| 6 | 分批翻译 | 每次 15-20 句一批 |
| 7 | 上下文窗口 | 前 5 句作为上下文 + 后续 20 句翻译（独立计数，每批发送 5+20=25 句） |
...
| 14 | 输入输出格式 | 输入纯文本 → 服务自动编号 → `ID: xxx \| 原文: xxx` → `ID: xxx \| 译文: xxx` |
| 15 | 断点续传 | 每批写入 .tmp（只保存译文 `ID: xxx \| 译文: xxx`），完成后 .tmp → `原文件名.zh.txt` |
...
| 目标语言 | **环境变量配置，默认中文** | `TARGET_LANG=zh` |
...
格式化后:
──────────────────────────────────
ID: 001 | 原文: Hello, how are you?
ID: 002 | 原文: I'm fine, thank you.
ID: 003 | 原文: What a beautiful day!
...
──────────────────────────────────
```
...
Batch 1 (Line 1~20):
  Context: (无，首批无前文)
  Translate: ID: 001~020 | 原文: ...
  → API Call → 追加写入 .tmp

Batch 2 (Line 21~40):
  Context: ID: 016~020 | 原文: ...   (前5句，仅给AI参考，不翻译)
  Translate: ID: 021~040 | 原文: ...
  → API Call → 追加写入 .tmp

Batch 3 (Line 41~60):
  Context: ID: 036~040 | 原文: ...
  Translate: ID: 041~060 | 原文: ...
  → API Call → 追加写入 .tmp
...
```
API 返回结果解析流程:
  1. 使用正则表达式逐行匹配: /^ID:\s*(\d+)\s*\|\s*译文:\s*(.+)$/
     → 捕获组1 = ID编号，捕获组2 = 译文内容（可能包含括号注释）
  2. 行数校验:
     发送 20 句 → 期望返回 20 行解析结果
     ├── 返回 20 行 → 正常写入 .tmp
     ├── 返回 < 20 行 → 该批次重试（不强行写入，防止时间轴错位）
     └── 返回 > 20 行 → 仅取前 20 行，日志记录异常
  3. ID 连续性校验:
     检查解析出的 ID 是否连续递增
     ├── 连续 → 正常
     └── 不连续 → 该批次重试
